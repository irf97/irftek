...
def _cboe_chain(j):
    ...
    return out
UA_PLAIN={"User-Agent":UA["User-Agent"],"Accept":"application/json"}
def fetch_chain(sess,t,want_src=False):
    for _ in range(2):                       # nasdaq, retried once
        try:
            ...
            if ch: return (ch,"nasdaq") if want_src else ch
        except Exception: pass
        time.sleep(1.0)
    try:  # CBOE delayed CDN — plain headers (nasdaq Origin/Referer can trip its WAF)
        j=sess.get(f"https://cdn.cboe.com/api/global/delayed_quotes/options/{t}.json",
                   headers=UA_PLAIN,timeout=15).json()
        ch=_cboe_chain(j)
        return (ch,"cboe" if ch else "none") if want_src else ch
    except Exception:
        return ([],"none") if want_src else []
def _hist_df(j):
...
def cmd_feed(tier):
    ...
    for t in names:
        ...
        ch,chsrc=fetch_chain(sess,t,want_src=True)
        if ch:
            px=(q or {}).get("px")
        ...
        print(f"[{t}] px={q['px'] if q and q.get('px') else '—'} ({q['src'] if q else 'no quote'})"
              ...
              f" · chain={len(feed['chains'].get(t,[]))} ({chsrc})")
        diag["chain_src"][chsrc]=diag["chain_src"].get(chsrc,0)+1
        if chsrc=="none": diag["chain_zero"].append(t)
        time.sleep(0.8)
